...
def _insert_message(conn, channel, tier, message_id, views, hours_ago=1, is_forward=0, text="Some claim about politics"):
    ...
    conn.execute(
        ...
        (channel, tier, message_id, date, text, views, is_forward, None, scraped_at),
    )
    conn.commit()
...
def test_get_top_messages_returns_sorted_by_virality(tmp_path):
    ...
    with patch.object(db, "DB_PATH", db_path):
        conn = db.get_conn()
        _insert_message(conn, "low_views", "tier1", 1, views=1000)
        _insert_message(conn, "high_views", "tier1", 2, views=50000)
        _insert_message(conn, "mid_views", "tier1", 3, views=10000)
...
def test_get_top_messages_deduplicates_same_text(tmp_path):
    ...
    with patch.object(db, "DB_PATH", db_path):
        ...
        _insert_message(conn, "chan_a", "tier1", 1, views=5000, text=shared_text)
        _insert_message(conn, "chan_b", "tier1", 2, views=3000, text=shared_text)
        _insert_message(conn, "chan_c", "tier1", 3, views=2000, text="Completely different claim here")
...
def test_get_top_messages_respects_hours_window(tmp_path):
    ...
    with patch.object(db, "DB_PATH", db_path):
        conn = db.get_conn()
        _insert_message(conn, "recent", "tier1", 1, views=5000, hours_ago=1)
        _insert_message(conn, "old", "tier1", 2, views=5000, hours_ago=48)
...
def test_get_top_messages_tier_filter(tmp_path):
    db_path = _setup_db(tmp_path)
    with patch.object(db, "DB_PATH", db_path):
        conn = db.get_conn()
        _insert_message(conn, "t1_chan", "tier1", 1, views=5000)
        _insert_message(conn, "t2_chan", "tier2", 2, views=5000)

        results = score.get_top_messages(hours=24, tier="tier1")

    channels = [r["channel"] for r in results]
    assert "t1_chan" in channels
    assert "t2_chan" not in channels
...
def test_get_top_messages_excludes_empty_text(tmp_path):
    ...
    with patch.object(db, "DB_PATH", db_path):
        conn = db.get_conn()
        _insert_message(conn, "no_text", "tier1", 1, views=10000, text="")
        _insert_message(conn, "has_text", "tier1", 2, views=5000, text="A real claim")
...
def test_get_random_messages_respects_hours_window(tmp_path):
    ...
    with patch.object(db, "DB_PATH", db_path):
        conn = db.get_conn()
        _insert_message(conn, "recent", "tier1", 1, views=100, hours_ago=1)
        _insert_message(conn, "old", "tier1", 2, views=100, hours_ago=48)
...
def test_get_random_messages_respects_limit(tmp_path):
    ...
    with patch.object(db, "DB_PATH", db_path):
        conn = db.get_conn()
        for i in range(10):
            _insert_message(conn, f"chan_{i}", "tier1", i, views=100)
...
def test_get_spotlight_messages_one_per_channel(tmp_path):
    ...
    with patch.object(db, "DB_PATH", db_path):
        conn = db.get_conn()
        _insert_message(conn, "chan_a", "tier1", 1, views=100)
        _insert_message(conn, "chan_a", "tier1", 2, views=5000)  # chan_a's top post
        _insert_message(conn, "chan_b", "tier1", 3, views=200)
...
def test_get_spotlight_messages_small_channel_not_crowded_out(tmp_path):
    ...
    with patch.object(db, "DB_PATH", db_path):
        ...
        for i in range(20):
            _insert_message(conn, "chan_big", "tier1", i, views=100000)
        # chan_small posts one modest message
        _insert_message(conn, "chan_small", "tier1", 100, views=50)
...
def test_get_spotlight_messages_per_channel_limit(tmp_path):
    ...
    with patch.object(db, "DB_PATH", db_path):
        conn = db.get_conn()
        for i in range(5):
            _insert_message(conn, "chan_a", "tier1", i, views=100 * i)
